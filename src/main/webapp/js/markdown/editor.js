(function(global) {
	...
	function toggleFullScreen(editor) {
		...
		var lastStyle = $(el).attr("style");
		var cancel = function() {
			$(el).attr("style", lastStyle);
			if (doc.cancelFullScreen) {
				doc.cancelFullScreen();
				window.document.removeEventListener("fullscreenchange", scFun);
			} else if (doc.mozCancelFullScreen) {
				doc.mozCancelFullScreen();
				window.document.removeEventListener("mozfullscreenchange", scFun);
			} else if (doc.webkitCancelFullScreen) {
				doc.webkitCancelFullScreen();
				window.document.removeEventListener("webkitfullscreenchange", scFun);
			}
		};
		var scFun = function() {
			if (!isFull()) {
		...
		var request = function() {// tbw 监听全局的全屏事件
			$(el).removeAttr("style");
			if (el.requestFullScreen) {
				el.requestFullScreen();
				window.document.addEventListener("fullscreenchange", scFun, false);
			} else if (el.mozRequestFullScreen) {
				el.mozRequestFullScreen();
				window.document.addEventListener("mozfullscreenchange", scFun, false);
			} else if (el.webkitRequestFullScreen) {
				el.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT);
				window.document.addEventListener("webkitfullscreenchange", scFun, false);
			}
		};
		if (!isFull()) {
			request();
	...
	function _toggleLine(cm, name) {
		...
		for (var i = startPoint.line; i <= endPoint.line; i++) {
			(function(i) {
				...
				cm.replaceRange(text,{line:i,ch:0},{line:i,ch:text.length});
			})(i);
		}
		cm.focus();
	}
	...
	var toolbar = [
	               ...
	               {name: 'quote', action: toggleBlockquote, className: 'fa fa-quote-right'},
	               ...
	               {name: 'image', action: drawImage, className: 'fa fa-image'},
	               '|',

	               {name: 'preview', action: togglePreview, className: 'fa fa-eye'},
	               {name: 'fullscreen', action: toggleFullScreen, className: 'fa fa-expand'}
	               ];
...
(function(document, window) {
	 ...
	 */
	inlineAttachment.prototype.uploadFile = function(file,isNotImage) {
		var me = this,
		...
		xhr.onload = function() {
			...
			if (xhr.status === 200 || xhr.status === 201) {
				me.onFileUploadResponse(xhr,isNotImage);
			} else {
	...
	inlineAttachment.prototype.isNotImageFile = function(file) {
	 ...
	 * @return {Void}
	 */
	inlineAttachment.prototype.onFileUploadResponse = function(xhr,isNotImage) {
	...
	inlineAttachment.prototype.onDrop = function(e) {
		...
		for (var i = 0; i < e.dataTransfer.files.length; i++) {
			...
			if (this.isFileAllowed(file)) {
				...
				this.uploadFile(file,this.isNotImageFile(file));
			}
		}
